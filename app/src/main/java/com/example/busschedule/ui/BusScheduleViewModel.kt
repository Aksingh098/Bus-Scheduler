...
class BusScheduleViewModel(private val repository: ScheduleRepository): ViewModel() {
    ...
    fun getFullSchedule(): Flow<List<BusSchedule>> = repository.getAllItemsStream()




    // Get example bus schedule by stop
    fun getScheduleFor(stopName: String): Flow<List<BusSchedule>> = repository.getItemStream(stopName)
   ...
   companion object {
    val Factory: ViewModelProvider.Factory = viewModelFactory {
        initializer {
            ...
            val repository = application.container.scheduleRepository
